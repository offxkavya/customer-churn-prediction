...
\begin{enumerate}
    \item \textbf{Streamlit application}: A lightweight, interactive web app (\texttt{app.py}, 89 lines) was built for real-time inference. It uses sidebar widgets for input and displays both the churn probability and a categorical risk level (HIGH/LOW).
...
\begin{tikzpicture}[
    ...
    \node[block, below=of preprocess] (model) {\textbf{Stage 3: Model Inference}\\Decision Tree Classifier (\texttt{max\_depth=5}) processes the transformed feature vector\\Outputs: binary prediction (0/1) + probability score};
    \node[block, below=of model] (output) {\textbf{Stage 4: Output Display}\\Churn probability (\%) with progress bar\\Risk level indicator (HIGH/LOW) with color coding\\Full customer profile summary table};
...
\subsection{Design Justification: Why Decision Tree Over Other Models}

Two models were evaluated. The Decision Tree was selected as the production model over Logistic Regression for the following reasons:

\begin{enumerate}
    \item \textbf{Non-linear decision boundaries}: Churn behavior is driven by complex interactions between features (e.g., high Payment Delay \textit{combined with} low Usage Frequency). Logistic Regression can only model linear relationships between features and the target, which is why it plateaued at 83.17\% accuracy. The Decision Tree captures these non-linear interactions naturally through recursive feature-space partitioning, achieving 95.97\%.

    \item \textbf{Built-in feature importance}: Unlike Logistic Regression (where coefficient interpretation requires careful handling of scaled features), the Decision Tree provides direct, intuitive feature importance scores based on how much each feature reduces impurity across all splits. This enabled us to immediately identify Payment Delay as the dominant churn driver (47.9\%).

    \item \textbf{No distributional assumptions}: Logistic Regression assumes a linear relationship between features and the log-odds of the target. The Decision Tree makes no such assumption, making it more robust to the mixed distributions present in our dataset.

    \item \textbf{Interpretability}: With \texttt{max\_depth=5}, the tree has at most 32 leaf nodes. Each prediction can be traced through a human-readable sequence of if-else rules, which is critical for explaining churn predictions to business stakeholders.

    \item \textbf{Generalization}: Despite its higher accuracy, the Decision Tree shows no overfitting (train: 95.63\%, test: 95.97\%), confirming that the depth constraint provides effective regularization.
\end{enumerate}

More complex models (Random Forest, XGBoost) were not used in Milestone\,1 because the Decision Tree already achieves $>$95\% accuracy with excellent interpretability. Ensemble methods are planned for Milestone\,2 if further accuracy gains are needed.
...
\subsection{scikit-learn (v1.4.2)}

scikit-learn is the core ML library used for preprocessing, model training, and evaluation. \textit{Why scikit-learn?} It provides a unified API where every estimator follows the same \texttt{fit()}/\texttt{predict()}/\texttt{transform()} pattern, making it easy to swap models or preprocessing steps without rewriting code. Key components used:
...
\begin{itemize}
    ...
    \item \texttt{DecisionTreeClassifier}: Tree-based classifier with configurable depth constraints.
    \item Metrics: \texttt{accuracy\_score}, \texttt{precision\_score}, \texttt{recall\_score}, \texttt{f1\_score}, \texttt{confusion\_matrix}.
\end{itemize}

\subsection{pandas (v2.2.2) \& NumPy (v1.26.4)}
...
\subsection{Streamlit (v1.33.0)}

Streamlit is a Python framework for building interactive web applications with minimal code. \textit{Why Streamlit?} Unlike Flask or Django, Streamlit requires no HTML/CSS/JavaScript knowledge. A complete interactive dashboard can be built in under 100 lines of Python. It provides built-in widgets (\texttt{slider}, \texttt{selectbox}, \texttt{number\_input}) that automatically handle user input, state management, and reactive re-rendering. This made it the ideal choice for rapid prototyping of the inference interface.

\subsection{joblib (v1.3.2)}

joblib is used for model serialization (saving and loading the trained pipeline). \textit{Why joblib over Python's built-in \texttt{pickle}?} joblib is optimized for objects containing NumPy arrays (which scikit-learn models internally use), providing faster serialization and built-in compression. We used \texttt{compress=3} to reduce the model file to just 3.6\,KB, enabling near-instant loading at application startup.
...
\subsection{Sub-Feature 2: Real-Time Inference Engine}

The Streamlit application provides \textbf{real-time, interactive predictions}. When a user adjusts any input widget and clicks ``Predict Churn,'' the system:
\begin{itemize}
    ...
    \item Returns both the binary classification and the continuous probability score within milliseconds.
\end{itemize}
This enables customer success teams to perform ad-hoc risk assessments without writing code or waiting for batch processing.

\subsection{Sub-Feature 3: Interactive Risk Dashboard}

The output interface goes beyond a simple yes/no prediction. It provides a \textbf{multi-component risk dashboard}:
\begin{itemize}
    \item \textbf{Churn probability meter}: A progress bar visualizing the exact probability (e.g., 73.2\%), giving a nuanced risk view.
    \item \textbf{Color-coded risk indicator}: Red (\texttt{HIGH}) or green (\texttt{LOW}) alert based on the binary prediction, enabling at-a-glance assessment.
    \item \textbf{Customer profile summary}: A formatted table displaying all input features, creating a self-contained record for each prediction.
\end{itemize}
...
The Streamlit application is deployed on \textbf{Hugging Face Spaces}, a free hosting platform for ML demos. \textit{Why Hugging Face Spaces?} It natively supports Streamlit apps, provides automatic builds from Git repositories, requires zero infrastructure management, and offers a public URL for easy sharing with stakeholders and evaluators. The deployment process is:
...
\begin{enumerate}
    \item The serialized pipeline (\texttt{churn\_model.pkl}) is loaded at startup using \texttt{joblib.load()}.
    ...
    \item \texttt{model.predict()} returns the binary churn prediction; \texttt{model.predict\_proba()} returns the probability.
    \item Results are displayed with a progress bar (probability), color-coded risk indicator, and a summary table.
\end{enumerate}
...
This project successfully demonstrates that customer churn can be predicted with high accuracy (95.97\%) using a depth-constrained Decision Tree classifier on behavioral data. The key finding is that \textbf{Payment Delay is the dominant churn signal}, followed by Support Calls and Tenure. The full pipeline---from data preprocessing through model deployment on Streamlit---is operational and serves real-time predictions.
